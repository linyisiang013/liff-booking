...
export default function LiffBookingPage() {
  ...
  const isAfter17th = today.getDate() >= 10;
    ...
    try {
      ...
      const res = await fetch("/api/bookings", {
        ...
        body: JSON.stringify({ 
          ...
          customer_phone: formData.phone, // 這裡存的是卸甲選項
          item: formData.item, 
          date: formData.date, 
      ...
      if (res.ok) {
        // 2. 資料庫成功後，使用 LIFF 幫客戶傳送訊息 (免費)
        if (liff.isInClient()) {
          try {
            await liff.sendMessages([{
              type: 'text',
              text: `【預約確認】\n姓名：${formData.name}\n日期：${formData.date}\n時間：${formData.slot_time}\n項目：${formData.item}\n卸甲：${formData.phone}`
            }]);
          } catch (msgError) {
            console.error("LIFF 傳訊失敗", msgError);
            // 就算傳訊失敗(例如沒加好友)，資料庫已寫入，還是算成功
          }
        }
        ...
        alert(err.error || "該時段已被搶先預約，請重選");
        // 重新讀取時段狀態
        const targetDate = formData.date;
        fetch(`/api/availability?date=${targetDate}&t=${Date.now()}`)
            .then(res => res.json())
            .then(data => setAvailabilityData({allSlots: data.allSlots, allDisabled: data.allDisabled}));
      }
    } catch (e) { 
      alert("連線失敗"); 
    } finally { 
    ...
    <div style={s.container}>
      ...
      <div style={s.card}>
        <div style={s.calendarHeader}>
          ...
          <button onClick={handleNextMonth} style={{...s.navBtn, opacity: (!isAfter17th && viewDate.getMonth() === today.getMonth()) ? 0.3 : 1}}>下月</button>
        </div>
